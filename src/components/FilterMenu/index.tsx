...
const handleFieldsChange = (e : {currentTarget : {value : string}}, setSearch : (value: React.SetStateAction<string>) => void,setCards: (value: React.SetStateAction<CardType[]>) => void, typeContent: string, moviesList: CardType[], seriesList: CardType[], animesList: CardType[])=>{
    setSearch(e.currentTarget.value)
    SearchCards(e,setCards,setSearch,typeContent, moviesList, seriesList, animesList)
}
...
const SearchCards = async (e : {currentTarget : {value : string}}, setCards: (value: React.SetStateAction<CardType[]>) => void, setSearch: (value: React.SetStateAction<string>) => void, typeContent: string, moviesList: CardType[], seriesList: CardType[], animesList: CardType[])=>{
    SearchCardsUtil(e,setCards,setSearch,typeContent, moviesList, seriesList, animesList)
}
    ...
    if(typeContent != "Search")
            ...
            <>             
                <section className="filterMenu container flex-space-around w-100" >
                            ...
                            <div className="inputSearch">
                                <input className="inputSearchTarget" type="text" id="search" name="search" value={search} onChange={(e)=>{handleFieldsChange(e,setSearch,setCards,typeContent, moviesList, seriesList, animesList)}}  onBlur={()=>{changeLabel(parentNode,search)}} placeholder=""/>
        ...
        <>
            <section className="filterMenu Search container" >
                            ...
                            <div className="inputSearch">
                                <input className="inputSearchTarget" type="text" id="search" value={search} onChange={(e)=>{handleFieldsChange(e,setSearch,setCards,typeContent, moviesList, seriesList, animesList)}}  onBlur={()=>{changeLabel(parentNode,search)}} placeholder=""/>
                                <label id="searchLabel" htmlFor="search">Digite o Título do Filmes, Serie...</label>
                            </div>  
            </section>    
        </>
